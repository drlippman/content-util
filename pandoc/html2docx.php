...
if (isset($_POST['convertsvg'])) {
	...
	$html = preg_replace_callback('|<svg.*?</svg>|sm','[IMAGE - REPLACE]', $html);
...
if (isset($_POST['convertsvg'])) {
	exec('/bin/pandoc '.$filename.'.html -f html+tex_math_double_backslash -o '.$filename.'.tex');
	exec('/bin/pandoc '.$filename.'.tex -o '.$filename.'.docx');
} else {
	exec('/bin/pandoc '.$filename.'.html -f html+tex_math_double_backslash -o '.$filename.'.docx');
}
